...
<table class="table table-hover">
    ...
    <thead>
        <tr style="height: 50px;">
            ...
            <!--STATUS-->
            <?php $name = 'field_status' ?>

            <th class="hidden-xs" style='width:{{ $withs['status'] }}'>{!! trans($plang_admin.'.columns.field-status') !!}
                <a href='{!! $sorting["url"][$name] !!}' class='tb-id' data-order='asc'>
                    @if($sorting['items'][$name] == 'asc')
                        <i class="fa fa-sort-alpha-asc" aria-hidden="true"></i>
                    @elseif($sorting['items'][$name] == 'desc')
        ...
        @foreach($items as $item)
            <tr>
                ...
                <td style="text-align: center;">
                    ...
                    @if($item->field_status && (isset($status['list'][$item->field_status])))
                        <i class="fa fa-circle" style="color:{!! $status['color'][$item->field_status] !!}" title='{!! $status["list"][$item->field_status] !!}'></i>
                    @else
                ...
                <td>
                    ...
                    <!--edit-->
                    <a href="{!! URL::route('fields.edit', [   'id' => $item->id,
                                                                '_token' => csrf_token()
                                                            ])
                            !!}">
                    ...
                    <!--copy-->
                    <a href="{!! URL::route('fields.copy',[    'cid' => $item->id,
                                                                '_token' => csrf_token(),
                                                            ])
                             !!}"
                        class="margin-left-5">
