<?php namespace Foostart\Field\Models;

use Foostart\Category\Library\Models\FooModel;
use Illuminate\Database\Eloquent\Model;
use Foostart\Comment\Models\Comment;

class Field extends FooModel {
    ...
    public function getComments($field_id) {

        // Get field
        $params = array(
            'id' => $field_id,
        );
        $field = $this->selectItem($params);
        ...
        $field->cache_comments = json_encode($users_comments);
            ...
            foreach($params as $column => $value)
            {
                if($this->isValidValue($value))
                {
                    switch($column)
                    {
                        ...
                        case '_id':
                            if (!empty($value)) {
                                $elo = $elo->where($this->table . '.field_id', '!=', $value);
                            }
                            break;
                        case 'status':
                            if (!empty($value)) {
                        ...
                        case 'keyword':
                            if (!empty($value)) {
                                $elo = $elo->where(function($elo) use ($value) {
                                    $elo->where($this->table . '.field_name', 'LIKE', "%{$value}%")
                                    ->orWhere($this->table . '.field_description','LIKE', "%{$value}%")
                                    ->orWhere($this->table . '.field_overview','LIKE', "%{$value}%");
                                });
                            }
                            break;
                        default:
    ...
    public function updateItem($params = [], $id = NULL) {
        ...
        //get field item by conditions
        $_params = [
            'id' => $id,
        ];
        $field = $this->selectItem($_params);
