<?php namespace Foostart\Field\Controllers\Admin;
...
use Foostart\Field\Validators\FieldValidator;


class FieldAdminController extends FooController {
    ...
    public function __construct(Request $request) {
        ...
        //models
        $this->obj_item = new Field(array('perPage' => 10));
        $this->obj_category = new Category();
        $this->obj_slideshow = new Slideshow();

        //validators
        $this->obj_validator = new FieldValidator();

        //set language files
        $this->plang_admin = 'field-admin';
        $this->plang_front = 'field-front';

        //package name
        $this->package_name = 'package-field';
        $this->package_base_name = 'field';

        //root routers
        $this->root_router = 'fields';
        ...
        $this->category_ref_name = 'admin/fields';
    ...
    public function index(Request $request) {
        ...
        if ($is_admin ) {
            ...
            return redirect()->route('fields.list', ['user_id' => $user['user_id']]);
    ...
    public function config(Request $request) {
        ...
        $is_valid_request = $this->isValidRequest($request);
        // display view
        $config_path = realpath(base_path('config/package-field.php'));
        $package_path = realpath(base_path('vendor/foostart/package-field'));
        ...
        if ($request->isMethod('post') && $is_valid_request) {
            ...
            file_put_contents($config_bakup.'/package-field-'.date('YmdHis',time()).'.php', $content);
    ...
    public function lang(Request $request) {
        ...
        $is_valid_request = $this->isValidRequest($request);
        // display view
        $langs = config('package-field.langs');
        $lang_paths = [];
        $package_path = realpath(base_path('vendor/foostart/package-field'));

        if (!empty($langs) && is_array($langs)) {
            foreach ($langs as $key => $value) {
                $lang_paths[$key] = realpath(base_path('resources/lang/'.$key.'/field-admin.php'));
        ...
        if ($request->isMethod('post') && $is_valid_request) {
            ...
            foreach ($lang_paths as $key => $value) {
                ...
                //format file name field-admin-YmdHis.php
                file_put_contents($lang_bakup.'/'.$key.'/field-admin-'.date('YmdHis',time()).'.php', $content);
            }
